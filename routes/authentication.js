...
async function SaveOTPToDB(fullname, birthday, education_level, location, email, hashed_password, token, otp, role) {
    ...
    if (check_exist.length > 0) {
        await db.query(
            `UPDATE OTP 
             SET fullname=?, birthday=?, education_level=?, location=?, hashed_password=?, token=?, otp=?, role=? 
             WHERE email=?`,
            [fullname, birthday, education_level, location, hashed_password, token, otp, role || 'user', email]
        );
    } else {
        console.log("Data: ", fullname, birthday, education_level, location, email, hashed_password, token, otp, role);
        await db.query(
            `INSERT INTO OTP (fullname, birthday, education_level, location, email, hashed_password, token, otp, role) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [fullname, birthday, education_level, location, email, hashed_password, token, otp, role || 'user']
        );
    }
...
router.post('/update-password', requireRole('user', 'admin'), async (req, res) => {
