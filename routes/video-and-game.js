...
router.get('/get-token-lesson', requireRole('user', 'admin'), async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            return res.status(200).json({
                success: true,
                message: "Don't have any data for this API"
            });
        }
        ...
        console.error("Error in get-token-lesson: ", error);
        res.status(500).json({
            success: false,
...
router.post('/submit-answer', requireRole('user', 'admin'), async (req, res) => {
    ...
    try {
        ...
        const [id_lesson] = await db.query('SELECT id FROM LESSON WHERE token = ?', [token])
        console.log('Right Answer: ', rightAns)
        console.log('Point: ', point)
        const [insert] = await db.query('INSERT INTO USER_LEARNING (email, id_lesson, score) VALUES (?, ?, ?)', [email, id_lesson.id, point])
        console.log('So hang thay doi: ', insert.affectedRows)
        res.status(200).json({
            success: true,
